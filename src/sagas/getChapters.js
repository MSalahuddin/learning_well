...
import {get_all_chapters} from '../config/WebServices';
import * as types from '../actions/ActionTypes';

import {success, failure} from '../actions/GetChapters';
...
function callRequest(data) {
  ...
  let url = `${get_all_chapters}`;
  return ApiSauce.post(url, formdata);
}
function* watchRequest() {
  while (true) {
    const {payload} = yield take(types.GET_CHAPTERS.REQUEST);
